...
if __name__ == "__main__":
    ...
    if os.path.isdir(options.report):
        onlyfiles = [
            f for f in os.listdir(options.report)
            if os.path.isfile(os.path.join(options.report, f))
        ]
        for report in onlyfiles:
            deanonymize_report(
                os.path.join(options.report, report), options.data,
                options.layout, options.output)

    if os.path.isfile(options.report):
        deanonymize_report(options.report, options.data, options.layout,
                           options.output)
